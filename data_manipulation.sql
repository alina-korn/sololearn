...
В этом уроке мы научимся решать несколько более сложную задачу - 
нам нужно найти среднее количество телефонных номеров, которые 
есть у клиентов в нашей таблице.
У каждого клиента есть 0, 1 или несколько номеров телефона.
SELECT * FROM Customers;
SELECT * FROM PhoneNumbers;

Чтобы вычислить среднее значение, нам нужно узнать количество 
телефонных номеров, которые есть у каждого клиента, а затем 
использовать функцию AVG на этом наборе данных.
Сначала давайте объединим таблицы:
SELECT C.id, C.firstname, C.lastname, PN.number, PN.type
FROM Customers AS C LEFT JOIN PhoneNumbers AS PN
ON C.id = PN.customer_id
ORDER BY C.id

Выбрать все названия продуктов из 
таблицы Products вместе с соответствующими записями категорий в 
таблице ProductCategories. Продукты, которые не имеют 
соответствующих записей категорий, также должны быть в результате.
SELECT P.name, P.category
FROM Products AS P
LEFT JOIN ProductCategories AS PC
ON P.id = PC.product_id

Теперь мы можем группировать данные на основании наших клиентов 
и определить количество телефонных номеров, которые есть у каждого из 
них:
...
Если бы мы использовали JOIN вместо LEFT JOIN, мы бы получили клиентов, 
у которых есть совпадающие номера

Теперь нам нужно найти среднее значение этих значений. 
Для этого нам нужен другой SELECT запрос по данным объединения.
Мы присвоили запросу псевдоним 'Numbers' и выбрали среднее значение столбца count из него.
Обрамляя запрос SELECT в скобки, мы можем дать ему имя и использовать его как обычную таблицу.
Также важно давать псевдонимы именам пользовательских столбцов, чтобы вы могли выбирать их.
...
Вывод урока
Основной вывод из этого урока заключается в том, что вы можете 
заключить запрос в скобки и дать ему имя, используя ключевое слово AS. 
Это позволяет нам использовать запрос как таблицу: выбирать из нее, 
использовать в JOINS, запускать агрегатные функции и так далее.

Викторина
Внешний ключ prod_id таблицы Orders ссылается на столбец id таблицы 
Products.
Создать запрос, который выбирает столбцы name и amount из Orders с 
соответствующими именами Product.
SELECT Orders.name, Orders.amount, Products.name
FROM Orders JOIN Products
ON Orders.prod_id = Products.id


Какой из следующих вариантов удаляет дубликаты записей при объединении таблиц?
UNION

Таблица Users содержит 5 записей, в то время как таблица Accounts пуста. 
Таблица Accounts ссылается на таблицу Users с использованием внешнего ключа.
Сколько строк будет в результате Users RIGHT JOIN Account ON на связанных столбцах?

В таблице A содержится 5 строк, в то время как в таблице B содержится 3 строки.
