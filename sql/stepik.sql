...
Санкт-Петербурга (фамилии и инициалы сотрудников, город ,  
длительность командировки в днях, при этом первый и последний 
день относится к периоду командировки). Последний столбец назвать 
Длительность. Информацию вывести в упорядоченном по убыванию 
длительности поездки, а потом по убыванию названий городов 
...
Вывести информацию о командировках сотрудника(ов), которые были 
самыми короткими по времени. В результат включить столбцы name, 
...
Вывести информацию о командировках, начало и конец которых 
относятся к одному месяцу (год может быть любой). В результат 
включить столбцы name, city, date_first, date_last. Строки 
отсортировать сначала  в алфавитном порядке по названию города, 
а затем по фамилии сотрудника
SELECT 
    name,  -- Фамилия и инициалы сотрудника
    city,  -- Город
    date_first,  -- Дата начала командировки
    date_last  -- Дата окончания командировки
FROM trip
WHERE MONTH(date_first) = MONTH(date_last)  -- Проверяем, что начало и конец в одном месяце
ORDER BY 
    city ASC,  -- Сортировка по названию города (алфавитный порядок)
    name ASC;  -- Сортировка по фамилии сотрудника (алфавитный порядок)
...
по убыванию количества, а потом в алфавитном порядке по названию месяца 
виде. Название столбцов – Месяц и Количество.
SELECT 
    MONTHNAME(date_first) AS Месяц,  -- Название месяца
    COUNT(*) AS Количество  -- Количество командировок
FROM trip
GROUP BY Месяц  -- Группируем по названию месяца
ORDER BY 
    Количество DESC,  -- Сортировка по убыванию количества командировок
    Месяц ASC;  -- Сортировка по алфавитному порядку названий месяцев

Вывести сумму суточных (произведение количества дней командировки и 
размера суточных) для командировок, первый день которых пришелся на 
февраль или март 2020 года. Значение суточных для каждой командировки 
занесено в столбец per_diem. Вывести фамилию и инициалы сотрудника, 
город, первый день командировки и сумму суточных. Последний столбец 
назвать Сумма. Информацию отсортировать сначала  в алфавитном порядке 
по фамилиям сотрудников, а затем по убыванию суммы суточных.
SELECT 
    name,  -- Фамилия и инициалы сотрудника
    city,  -- Город
    date_first,  -- Первый день командировки
    (DATEDIFF(date_last, date_first) + 1) * per_diem AS Сумма  -- Сумма суточных
FROM trip
WHERE 
    (MONTH(date_first) = 2 AND YEAR(date_first) = 2020)  -- Февраль 2020
    OR 
    (MONTH(date_first) = 3 AND YEAR(date_first) = 2020)  -- Март 2020
ORDER BY 
    name ASC,  -- Сортировка по фамилии сотрудника (алфавитный порядок)
    Сумма DESC;  -- Сортировка по убыванию суммы суточных

Вывести фамилию с инициалами и общую сумму суточных, полученных за все 
командировки для тех сотрудников, которые были в командировках 
больше чем 3 раза, в отсортированном по убыванию сумм суточных виде. 
Последний столбец назвать Сумма.
SELECT 
    name,  -- Фамилия и инициалы сотрудника
    SUM((DATEDIFF(date_last, date_first) + 1) * per_diem) AS Сумма  -- Общая сумма суточных
FROM trip
WHERE name IN (
    SELECT name
    FROM trip
    GROUP BY name
    HAVING COUNT(*) > 3  -- Сотрудники с более чем 3 командировками
)
GROUP BY name
ORDER BY Сумма DESC;  -- Сортировка по убыванию суммы суточных

## 1.7
CREATE TABLE fine (
    fine_id INT AUTO_INCREMENT PRIMARY KEY,  -- Ключевой столбец с автоматическим увеличением
    name VARCHAR(30),  -- Строка длиной 30
    number_plate VARCHAR(6),  -- Строка длиной 6
    violation VARCHAR(50),  -- Строка длиной 50
    sum_fine DECIMAL(8, 2),  -- Вещественное число с 2 знаками после запятой
    date_violation DATE,  -- Дата нарушения
...
в соответствии с данными из таблицы traffic_violation. При этом суммы 
заносить только в пустые поля столбца  sum_fine
UPDATE 
    fine f, 
...
WHERE 
    f.violation = tv.violation  -- Сравниваем нарушения
    AND f.sum_fine IS NULL;     -- Обновляем только пустые значения
...
GROUP BY 
    ...
    number_plate, 
    violation
HAVING COUNT(*) >= 2  -- Находим водителей с двумя и более одинаковыми нарушениями
ORDER BY 
    name ASC,  -- Сортировка по фамилии
    number_plate ASC,  -- Сортировка по номеру машины
    violation ASC;  -- Сортировка по нарушению
...
JOIN (
    ...
    GROUP BY 
        ...
        violation
    HAVING COUNT(*) >= 2  -- Находим водителей с двумя и более одинаковыми нарушениями
) AS repeated_violations
ON 
    f.name = repeated_violations.name
    AND f.number_plate = repeated_violations.number_plate
    AND f.violation = repeated_violations.violation
SET 
